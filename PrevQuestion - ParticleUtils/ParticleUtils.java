...
@SuppressWarnings("unused") // Suppress IDE warnings for unused variables
public class ParticleUtils {
    @Getter // External library automatically makes a function to get this variable below
    private static final double[][] sphere = new double[7 * 6 * 2][]; // static means the field is not associated with any particular instance of the class final means it is a constant variable and it cannot be changed
    // The variable is a 2d float array with 84 rows and 0 columns

    static { // Static block is executed once when the class first loads, usually used to set static variables
        int sphereLoc = 0;
        for (double i = 0; i <= Math.PI; i += Math.PI / 6) { // Run this loop 7 times
            final double radius = Math.sin(i);
            final double y = Math.cos(i);
            for (double a = 0; a < Math.PI * 2; a+= Math.PI / 6) { // Run this loop 12 times
                final double x = Math.cos(a) * radius;
                final double z = Math.sin(a) * radius;
                sphere[sphereLoc] = new double[] { x, y, z }; // Ends up filling all the columns with another array(representing a point) using 3 float coordinates
                sphereLoc++;
            }
        }
    }

    private static Vector generateRandomOffset(boolean setRandom) { // static function is not associated with any specific class instance
        return setRandom ? new Vector(RandomUtils.randomDouble(), RandomUtils.randomDouble(), RandomUtils.randomDouble()) : new Vector(); // if setRandom is true, return a vector with all random floats, otherwise return a blank vector
    }

    public static void randomParticle(Location location, Particle particle, double radius, int count) { // Wrapper function to accept different arguments
        randomParticle(location, particle, radius, radius, radius, count);
    }

    public static void randomParticle(Location location, Particle particle, double xRadius, double yRadius, double zRadius, int count) { // Wrapper function
        for (int i = 0; i < count; ++i) {
            randomParticle(location, particle, xRadius, yRadius, zRadius);
        }
    }

    public static void randomParticle(Location location, Particle particle, double radius) { // Wrapper function
    ...
    @ParametersAreNonnullByDefault // make sure parameters of this function are not null pointers
